...
static void test_Reduce() {
             ...
             {
                         const auto a =
                                            Tensor<float>::arange(0,120,1)
                                            ->view({4,5,6})->slice({":2","1:","3:"});
                         const std::vector<std::vector<float>> expected = {
                             { 76545.,   98560.,  124729.},
                             {5101785., 5549440., 6025729.},
                         };
                         auto b = a->prod({1});
                         const auto tensor = std::dynamic_pointer_cast<Tensor<float>>(b);
                         BREEZE_ASSERT(tensor->get_shape().dims() == std::vector<index_t>({2,3}));
            ...
            //测试  arg_max
             {
                         ...
                         auto b = a->arg_max({1},false);
                         const auto tensor = std::dynamic_pointer_cast<Tensor<index_t>>(b);
                         BREEZE_ASSERT(tensor->get_shape().dims() == std::vector<index_t>({2,3}));
                         COMPARE_TENSOR_DATA(tensor->mutable_data(), expected, 1e-6);
             }
            ...
            //测试  arg_min
            {
                 ...
                 auto b = a->arg_min({1},false);
                 const auto tensor = std::dynamic_pointer_cast<Tensor<index_t>>(b);
                 BREEZE_ASSERT(tensor->get_shape().dims() == std::vector<index_t>({4,6}));
                 COMPARE_TENSOR_DATA(tensor->mutable_data(), expected, 1e-6);
            }
